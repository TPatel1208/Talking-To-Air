...
def lookup_by_handle(handle: str) -> Any | None:
    ...
    **not** relaxed in exchange — the same per-hit integrity sweep T52 runs on
    ...
    """
    from tta_backend.utils.metrics import record_cache_hit, record_cube_index_hit, record_cube_index_miss
...
def reconcile_handle(handle: str, key: str) -> None:
    ...
    **Why catching drift here is sufficient, given that an index hit never gets
    ...
    """
    from tta_backend.utils.metrics import record_cube_index_invalidation
...
def evict_to_fit(incoming_bytes: int) -> int:
    ...
    """
    from tta_backend.utils.metrics import record_cube_eviction
